...
| `npm run check:docs` | Runs every documentation validator the installed `living-project-documentation` skill ships against [`docs/`](docs/index.md), and fails the gate if any of them cannot run. Needs no `npm install`. | After adding or removing a document under `docs/`, editing one's links, adding a spec, or writing or superseding a decision record. |
...
`npm run lint`, `npm run typecheck`, `npm run test:unit`, `npm run check:docs`,
and the **Payload Artifacts** drift check are the five checks CI gates a merge
on; the e2e suite and its scenario-coverage gate run after merge, on `main`.
