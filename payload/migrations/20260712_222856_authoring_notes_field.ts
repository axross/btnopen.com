...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  await db.run(sql`ALTER TABLE \`blog_posts\` ADD \`authoring_notes\` text;`)
  ...
  await db.run(sql`ALTER TABLE \`_blog_posts_v\` ADD \`version_authoring_notes\` text;`)
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  await db.run(sql`ALTER TABLE \`blog_posts\` DROP COLUMN \`authoring_notes\`;`)
  ...
  await db.run(sql`ALTER TABLE \`_blog_posts_v\` DROP COLUMN \`version_authoring_notes\`;`)
}
