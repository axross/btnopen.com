...
- A closed or errored question stream is **not** proof that the surface cannot render the UI. On remote, web, and cloud sessions the stream often closes because no human is answering *this* turn, yet the same human answers once you ask again. Treat a failed call as "**couldn't get a synchronous answer this turn**" — a possibly-transient condition — never as a permanent property of the surface, and never as grounds to discard an answer the human may still give. Do not diagnose what the surface supports; treat the error as a signal to **re-present and ask again** (next bullet), never to route around the human.
- MUST, when the question call errors (or a synchronous answer is otherwise unavailable), **re-present the decision and wait** — never route around the human. Show the decision in plain text first — background, the question, and the numbered options with the recommended default marked — then call the question tool again with those same options and hold for the answer. Do NOT post a marked GitHub "decision recorded here" comment, and do NOT end the turn as if blocked-with-a-breadcrumb. This holds whether or not the run is anchored yet: before any GitHub thread exists (e.g. a pre-anchor take-over question), state the question in the turn output and wait just the same. A closed or errored stream means *re-present and wait*, not *fall back and end the turn*.
- MUST, on the next turn, treat a bare answer token — an option label, an option number, or a short "Other" reply — as the human answering the still-open question, not a new instruction. Match it back to the options exactly as shown in that plain-text presentation (which is why the presentation must preserve their order and numbering) and resume from the pending step.
