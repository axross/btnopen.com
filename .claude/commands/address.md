...
Make all GitHub reads and writes per [GitHub Operations](../skills/github-operations/SKILL.md): the harness's sanctioned tool channel, acting as the connected operator, `@axross`, with the project's agent-comment marker (the single marker string recorded in that skill) on every comment you post so `/address continue` never mistakes your own output for human input. Follow [Development Guidelines](../skills/development-guidelines/SKILL.md) and the [Response Approach](../../AGENTS.md) workflow inside every phase.
...
- **A human decision — a Phase 1 Must-ask, plan approval, an ambiguous finding, or a conflict judgment call:** ask it through the dedicated question UI and use the answer inline (see [Asking the Human](#asking-the-human)). When the question tool does not return an answer this turn (the stream closes, or a headless run has no human answering), or when a machine event is stuck, post your state, **end the turn**, and wait at zero cost; the human resumes by sending **`/address continue`** — or a bare answer to your open question — in this same session.

- MUST poll autonomously ONLY for machine events (CI, the review workflow); never keep a session alive polling for a human.
- MUST resolve human decisions through the question UI inline when it returns an answer this turn (see [Asking the Human](#asking-the-human)); end the turn for `/address continue` when the question stream closes without an answer or a machine event is stuck, and never schedule a wake-up to re-check for human input.
...
Every human-gated **decision** in this flow — a Phase 1 Must-ask, plan approval under `--review-plan`, an ambiguous review finding, a conflict-resolution judgment call, or a take-over decision (which package, a zip↔inventory mismatch, a diverged precondition) — is asked through the harness's dedicated question tool (in Claude Code, **`AskUserQuestion`**): it renders your options as selectable choices in the chat and, when the human answers this turn, returns the answer inline, so the run continues in the same turn instead of ending and waiting for `/address continue`.

- MUST ask through the question tool first: frame the decision as concrete options (2–4), state the **default you would otherwise take** and mark it recommended, and rely on the tool's built-in "Other" choice for anything unanticipated. Never bury a decision in prose or silently assume an answer.
- A closed or errored question stream is **not** proof that the surface cannot render the UI. On remote, web, and cloud sessions the stream often closes because no human is answering *this* turn, yet the same human answers asynchronously on a later turn. Treat a failed call as "**couldn't get a synchronous answer this turn**" — a possibly-transient condition — never as a permanent property of the surface, and never as grounds to discard an answer the human may still give. Do not diagnose what the surface supports; state only that no synchronous answer arrived.
- MUST, when the question call does not return an answer this turn, record the **same decision** as a marked GitHub comment that @mentions `@axross` — the identical options in the identical order, numbering, and marked default as the question tool call — then **end the turn**. Phrase it as "couldn't get a synchronous answer, decision recorded here," not as a diagnosis of surface limitations. The comment is a durable breadcrumb `/address continue` resumes from, so the run never stalls. Exception: a pre-anchor take-over question (no issue or pull request exists yet, so there is no GitHub thread to write the breadcrumb to) — state the open question in the turn output, end the turn, and wait in-session instead.
- MUST, on the next turn, treat a bare answer token — an option label, an option number, or a short "Other" reply — as the human answering the still-open question, not a new instruction. Match it back to the options exactly as posted (which is why the fallback comment must preserve their order and numbering) and resume from the pending step.
