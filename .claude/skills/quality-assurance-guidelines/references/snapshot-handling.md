...
- CI (`check-and-deploy.yaml`) runs `npx playwright test --update-snapshots` and auto-creates a snapshot PR on a `snapshot-updates/<branch>` branch. That auto-PR still needs review for visual intent.
