...
Apply these rules to verify the author exercised the change in the dev server before relying on the e2e suite. Manual verification is the **first line** of confirmation per [development-guidelines › verification](../../development-guidelines/references/verification.md).
...
Dev and production diverge on transforms, caching, and asset handling, so a route that works under `npm run dev` can still break once built.
