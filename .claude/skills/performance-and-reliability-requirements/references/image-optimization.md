...
Images dominate page weight, and a raw `<img>` tag ships one fixed file to every device regardless of viewport, pixel density, or format support.
...
The image-processing backend fails soft: uploads keep succeeding while silently skipping conversion and resizing, so its loss shows up as slow pages rather than errors. The project's `payload/config.ts` passes `sharp` to Payload for image processing.
