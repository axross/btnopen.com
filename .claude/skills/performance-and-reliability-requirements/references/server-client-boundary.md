...
Apply these rules to verify Server / Client component composition does not create avoidable latency or ship unnecessary client JS.
...
A `<Suspense>` boundary streams at the pace of the slowest thing inside it, so grouping independent units forces the fast one to wait for the slow one.
...
Mixing loading and loaded concerns in one component couples the skeleton to data that does not exist yet, which is how skeletons quietly stop rendering before the fetch resolves.
...
The `"use client"` boundary is transitive — a promoted component carries every import beneath it — so one event handler can drag an entire subtree and its dependencies into the bundle.
...
- A Server Component that reads `cookies()` / `headers()` / `searchParams` switches the parent to dynamic rendering. Flag a Major when a previously-static route gains a new request-bound read in an unexpected place (e.g., a deep child), silently opting the whole route out of static rendering.
