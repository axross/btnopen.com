...
- MUST keep every step traceable to an acceptance criterion or a verification gate; a step that verifies nothing the plan states is scope creep in disguise.
...
- SHOULD note the environment specifics (route, data state, viewport, theme) when the defect only reproduces under them.

## Verification Gates

"Done" includes passing the project's own gates, not only the feature behavior. The gates the changed surface requires — format/lint, unit tests, e2e tests, build — are defined by [AGENTS.md › Verification](../../../../AGENTS.md#verification) with the project's development guidelines (verification rules) covering the command-level detail; this section names which of them apply to the change and why any is skipped.

**Guidelines:**

- MUST list the verification gates the changed surface requires as trailing steps, per [AGENTS.md › Verification](../../../../AGENTS.md#verification).
- MUST state why a normally-expected gate is skipped (e.g., "docs-only change — unit/e2e suites and build are unaffected"), never skip one silently.

## Coverage to Add or Update

The plan names the test coverage the work adds or updates — which suites, which behaviors — so the reviewer can check the coverage landed, not just that existing tests still pass. How those tests are written belongs to the project's unit-test guidelines and e2e testing guidelines; this section only names the coverage at spec level.

**Guidelines:**

- MUST name the test coverage to add or update as verification steps — the suite (unit, e2e) and the behavior each addition pins down.
- MUST defer test-writing mechanics (naming, structure, locators, fixtures) to the project's unit-test guidelines and e2e testing guidelines rather than restating them here.
