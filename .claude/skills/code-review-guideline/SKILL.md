...
when_to_use: Apply at the start of EVERY review task, including post-implementation self-review. Use for "review", "PR", "audit", "check this change", or self-review prompts. This owns review method (severity, evidence, reporting); domain criteria live in the per-topic lenses — e.g. application-security-requirements (security), performance-and-reliability-requirements (runtime cost), maintainable-code-guidelines (design), quality-assurance-guidelines (verification adequacy).
