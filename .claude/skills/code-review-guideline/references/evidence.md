...
````
[CRITICAL] app/(app)/_/repositories/get-blog-post.ts:53 — Unsanitized `slug` used in Payload `where` clause without an explicit string assertion.
Risk: A non-string `slug` (e.g., an array via search-param coercion) bypasses the equals filter and could leak draft content.
Fix:
```ts
- where: {
- 	slug: { equals: slug },
...
8. **Recommended Actions** — ordered checklist the author MUST complete before re-requesting review (e.g., "Run `npm run lint` after applying fix #1", "Re-run `npm run test:e2e` and confirm the affected `posts/[slug]` snapshots are unchanged").
