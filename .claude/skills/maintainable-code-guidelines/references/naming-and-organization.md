...
- MUST flag any new `.ts` / `.tsx` / `.module.css` file whose name is not kebab-case (e.g., `BlogPostHeader.tsx` or `blog_post_header.tsx`). The project rule is in the project's React component guidelines (conventions rules).
...
Place shared logic at the lowest tier that has more than one caller. The project has three tiers, plus a separate realm for the Payload data layer. The reviewer MUST verify a new module lives at the **lowest** tier that has more than one caller:
...
- MUST flag any new file placed under `app/(payload)/` — that directory is owned by Payload CMS per the project's development guidelines (change-management rules).
...
When the diff adds or moves a route, the reviewer MUST verify per the project's routing guidelines:
...
- MUST verify added or moved route files against the project's routing guidelines before approving their placement.
...
- MUST flag identifier-naming inconsistency within the changed file's neighborhood. Examples to flag:
  ...
  - A child logger module emoji that duplicates an existing module's emoji (the project uses unique emojis per module per the project's observability guidelines (logging rules)).
- SHOULD flag opaque abbreviations in new identifiers (`bp` for "blog post", `usr` for "user"). The project favors full words.
- SHOULD flag a `Promise<T>` prop received without the `Promise` suffix alias at the receiving component per the project's React component guidelines (client-vs-server-components rules).
