...
Both routes in the section below apply here, because this is a deviation and a gap at once. The deviation is the declined substitute. The gap is that the capability states the MUST without acknowledging that the vendor supplies no mechanism under what is now Next.js's default bundler, so a Turbopack project cannot comply through the SDK at all — which generalizes past this repository and is therefore a candidate for an upstream issue, pending the maintainer's go-ahead.
