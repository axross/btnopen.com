...
Values reach the browser or the server through exactly one barrel, so a review only has to look in one place to know what is exposed.
...
- MUST read environment values through `app/(app)/_/runtime.ts` from any component, repository, helper, route handler, or Payload collection. Biome's `noProcessEnv` whitelists only the four files above, so a direct `process.env` access elsewhere fails lint.
...
- MUST keep every `next.config.ts` `images.remotePatterns` entry scoped to a single origin with a path scope. The existing entries — `http://localhost:3000/api/**` and `https://cdn.hashnode.com/res/hashnode/**` — are the pattern; a wildcard hostname (`**`) is not.
...
- MUST compare a `searchParams` flag by value, not truthiness — `searchParams.draft === "true"`, as `app/(app)/_/repositories/get-blog-post.ts` does. A truthy check treats `?draft=false` as enabled.
