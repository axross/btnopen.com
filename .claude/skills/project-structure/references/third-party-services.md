...
| --- | --- | --- |
| `sendDefaultPii: true` | `sentry.server.config.ts`, `sentry.edge.config.ts`, `instrumentation-client.ts` | Sentry already captures IP addresses, request bodies, and user agents. This is the deprecated single-boolean form, which turns on every category at once; the current SDK expresses the posture as a structured `dataCollection` option instead — see the Sentry instrumentation capability |
...
- SHOULD keep third-party service inventory here instead of duplicating it in `CLAUDE.md`, which is deliberately limited to how agents work in this repository.
