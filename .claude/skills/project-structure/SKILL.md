...
description: Locating a file, placing or naming a module, or resolving any "how does THIS project do it" question a general capability defers to the project on — where data access lives, which tier a module belongs in, how a route directory is laid out, how a component splits across the server/client boundary, how Payload gates access and bounds queries, how a commenter is authenticated, how environment values are read, which logger and level to use, how source comments are cased, how tests and `data-testid`s are named, and where a collision with an installed skill is recorded. The structure and conventions of this Next.js + Payload CMS blog — the facts a shared skill library cannot know about it. Pair it with the general capability for the surface being changed, which owns the underlying practice.
