...
description: Writing, reviewing, or modifying this project's markdown rendering pipeline — `app/(app)/_/helpers/markdown.ts`, `app/(app)/_/components/markdown.tsx`, and the `Markdown`/`renderMarkdown` server components — even when the prompt says only "remark", "rehype", "mdast", "syntax highlighting", "blog post rendering", "embed", "webembed", "rich-text block", or a markdown bug. Also when judging whether authored content can inject markup here; this skill owns that surface's XSS rules, the link-protocol allowlist and body-link isolation included. Covers the plugin chain and its ordering rules, custom MDAST directive nodes, the `embed` directive and its Payload rich-text block, partial GFM, HAST-to-React mapping, Shiki, and server-only caching.
