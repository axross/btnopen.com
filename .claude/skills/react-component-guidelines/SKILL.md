...
when_to_use: Use when writing, reviewing, or refactoring React components — "component", "use client", "RSC", "Suspense", "styling", "container query", "oklch", "@scope", "animation-timeline", or "data-testid". Not for design/abstraction judgment (the maintainable code guidelines), runtime-cost or caching correctness (the performance and reliability requirements), or route/file placement (the routing guidelines and project structure guide).
