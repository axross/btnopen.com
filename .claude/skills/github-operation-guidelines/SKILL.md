...
- MUST begin every agent comment with this project's **one** fixed HTML marker line — `<!-- agent -->` — reused identically across every run and session.
...
The MUST bullets are non-negotiable; the SHOULD bullets are this project's default delivery conventions.

**Guidelines:**

- MUST NOT push to the default branch; work on this project's push-allowed branch prefix — `claude/`-prefixed branches.
...
- SHOULD open a pull request in **draft** while work is in progress and leave merging to the maintainer, @axross.
