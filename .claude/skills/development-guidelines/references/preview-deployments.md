...
- MUST set `PAYLOAD_TEST_USER_EMAIL` and `PAYLOAD_TEST_USER_PASSWORD` so the app self-seeds on first boot. They carry the **same value across all environments** (local, Preview, Production); the `onInit` seed is idempotent, so it provisions that admin account once per database and is a no-op wherever it already exists. Because the shared login also unlocks the Production admin and is reachable through the public preview, it MUST be a strong secret managed like any production credential (see the application-security requirements, privacy-and-exposure rules), never a throwaway.
...
- MUST set `PAYLOAD_TEST_USER_EMAIL` / `PAYLOAD_TEST_USER_PASSWORD` with the **same value across all Vercel environments** so every deployment self-seeds; use a strong secret, since the shared login also guards Production (see [Seeding](#seeding)).
