...
description: Designing, building, or reviewing any user-facing surface of btnopen.com — choosing a color role, radius, duration, or type tier; deciding responsive behavior; writing or reviewing a CSS Module; writing UI copy or an accessible name; building a loading, empty, or not-found state. The site's visual identity and the CSS that encodes it — the design language, color ramps and light/dark theming, breakpoints and reading-column widths, Japanese-primary copy, and the accessibility commitments. Pair it with the general high-fidelity UI design capability, which owns craft such as contrast targets, and the React component styling capability, which owns styling mechanics in the abstract.
