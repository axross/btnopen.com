...
- MUST NOT suffix non-callable subjects with `()`: schemas, codecs, object contracts, and type names stay bare, such as `describe("McpBlogPostResponse")`, and UI-component subjects take angle brackets instead, such as `describe("<BlogPostHeader>")`, when the unit suite renders components.
- MUST write `it(...)` names as sentence-like behavior statements that describe what the scenario verifies or ensures.
- SHOULD NOT repeat the outer `describe(...)` subject in every child `it(...)` name.
