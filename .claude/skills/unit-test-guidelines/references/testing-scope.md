...
- SHOULD use unit tests for boundary-value logic, schema defaults, validation failures, mutation safety, and custom MCP tool handlers exercised with faked Payload requests.
...
Components that accept callbacks or render children — render-prop components especially — are easy to test badly because the tempting path is to extract the callback or mock the dependency that drives it. That tests implementation wiring instead of user-visible behavior.
