...
Payload ships its own admin UI under `app/(payload)/` — that route segment is owned by the tool. Review focuses on respecting that boundary and on not mistaking UI affordances for access control.
