...
- MAY have a workflow entry-point skill delegate a compute-heavy, session-context-free phase to a named Workflow-tool script at `.claude/workflows/<name>.js` — a harness orchestration file, not a skill: it carries `export const meta` (whose `name` MUST match the filename) instead of frontmatter and gets no skill-index row, though the harness lists it beside skills, so its `meta.whenToUse` MUST carry the same do-not-invoke boundary as an entry point's `when_to_use`. The delegating skill keeps every human gate, external write, and verification command in its own driver text, MUST state an inline fallback for sessions without the Workflow tool (a failed run falls back, never passes), and names the script in the phase text that launches it.
