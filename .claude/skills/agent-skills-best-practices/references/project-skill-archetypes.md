...
Apply this reference when creating or extending the project's archetype skills — a structure skill, a component skill, and a UI/design skill; in this repository, [project-structure](../../project-structure/SKILL.md), [react-component-guidelines](../../react-component-guidelines/SKILL.md), and [ui-design-principles](../../ui-design-principles/SKILL.md) — or when auditing whether an existing project skill covers the topics its archetype expects.

## The Ownership Triangle

The three archetype skills divide one surface without overlap: the structure skill owns where files live, the component skill owns how surfaces are built, and the UI/design skill owns how they look. When each skill's opening paragraph names what its neighbors own, an agent that loads the wrong skill is rerouted immediately instead of finding duplicated rules.
...
- MUST name, in each skill's opening paragraph, which neighboring skill owns the adjacent concerns, with relative links.
- MUST keep the detailed wording of a shared rule in one owning skill and let neighbors link to it, per [cross-referencing.md](./cross-referencing.md).
...
2. **Stack** — factual bullets: runtime/framework, language, package manager, lint/format tooling, test tooling, styling approach.
...
- MUST cover component anatomy including test-id hooks, and cross-link the unit-test and e2e skills for how those hooks are consumed.
