...
The security trade-offs of this design — a publicly reachable preview serving fixture content rather than production data — are recorded in [Data Isolation and Exposure](#data-isolation-and-exposure) below; read it before changing what data a preview receives or who can reach it, alongside the application security capability for the general lens.
...
- MUST keep the Preview seed credential **distinct from Production's** — the preview admin login is publicly reachable, so reusing the Production value there would expose Production admin access (see [Data Isolation and Exposure](#data-isolation-and-exposure)).
...
- SHOULD use a distinct Preview `PAYLOAD_SECRET`, so preview session cookies and tokens cannot interoperate with production (see [Data Isolation and Exposure](#data-isolation-and-exposure)).
