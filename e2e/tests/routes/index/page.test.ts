...
test("Introduction section", {
	tag: ["@scenario:index.intro", "@area:index", "@priority:must", "@smoke"],
}, async ({ page }) => {
...
test("Landmark regions", {
	tag: ["@scenario:index.landmarks", "@area:index", "@priority:should"],
}, async ({ page }) => {
...
test(
	"Heading structure and list semantics",
	{
		tag: [
			"@scenario:index.heading-structure",
			"@area:index",
			"@priority:should",
		],
	},
...
test(
	"JSON-LD metadata",
	{ tag: ["@scenario:index.json-ld", "@area:metadata", "@priority:should"] },
	async ({ page }, testInfo) => {
		let website: Awaited<ReturnType<typeof getWebsite>>;
...
test(
	"Open Graph metadata",
	{ tag: ["@scenario:index.open-graph", "@area:metadata", "@priority:should"] },
	async ({ page }, testInfo) => {
		let website: Awaited<ReturnType<typeof getWebsite>>;
