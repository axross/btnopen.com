...
- MUST replace the default browser focus ring on interactive surfaces with the project's canonical `:focus-visible` pattern, not remove it outright. The design-side requirement is in [ui-design-principles › accessibility › keyboard-focus](../ui-design-principles/accessibility.md#keyboard-focus).
