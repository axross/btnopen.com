...
`"use cache"` Placement review focuses on critical-severity cases where `"use cache"` appears in a Client Component file (one with `"use client"`). Caching is a server-only directive.
...
`cacheLife` Token Choice review focuses on critical-severity cases where `"use cache"` appears without a `cacheLife("seconds" | "minutes" | "hours" | "days" | "weeks")` call immediately after. The default cache lifetime is unsafe to assume.
...
Invalidation Wiring review focuses on critical-severity cases where a new collection is added under `payload/collections/` that backs cached server reads, but no corresponding cache-invalidation `afterOperation` hook is added. Without it, edits in the admin UI leave stale UI for up to one cache lifetime.
...
Per-Path Revalidation review focuses on major-severity cases where `revalidatePath(path)` is called without the second `"page" | "layout"` argument when the change affects only one or the other. Wrong scope either over-invalidates (perf) or under-invalidates (stale layout).
...
Webembed Cache Specifics captures the project-specific context for the checklist below: The `getWebEmbedMetadata` function is `cacheLife("hours")`. The reviewer MUST flag a Critical when a diff makes this function vary by request-time inputs (e.g., adds a `User-Agent` parameter that changes per-visitor) — that explodes the cache key.
