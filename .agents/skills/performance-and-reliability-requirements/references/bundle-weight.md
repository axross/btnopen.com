...
Server-Only Imports in Client Files review focuses on critical-severity cases where a `"use client"` file imports any of these (each pulls in tens to hundreds of KB or breaks the build):
...
Heavy Client Dependencies review focuses on major-severity cases where a `"use client"` file imports a heavyweight library that has a smaller server-side equivalent. Examples observed in this project:
...
Re-Exports and Barrel Files review focuses on critical-severity cases where a new file is created that re-exports many items from a barrel (`index.ts` that re-exports everything) and is imported from a `"use client"` file. Per [development-guidelines › code-quality](../../development-guidelines/references/code-quality.md), the project rule is to import directly from the source module.
...
Dynamic Import Patterns describes the preferred project default: flag a Minor recommendation to `next/dynamic` (with `ssr: false` when appropriate) when a new Client Component is large, only used on a single route, and not above the fold.
...
Tree-Shaking review focuses on major-severity cases where a `"use client"` file uses a default import (`import _ from "lodash"`) instead of named imports for a library that supports tree-shaking. Default imports often defeat tree-shaking for CommonJS-shaped libraries.
