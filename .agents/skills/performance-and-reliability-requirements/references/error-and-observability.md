...
`try`/`catch` Placement review focuses on major-severity cases where a new `try`/`catch` is placed inside a nested helper rather than at the root call site (route handler, RSC, `route.ts`, server action). The project rule per [observability-guidelines › error-handling](../../observability-guidelines/references/error-handling.md) is "let errors propagate to the root call site".
...
Logger Discipline review focuses on critical-severity cases where the diff calls `logger.error(…)` — the project bans this in favor of `captureException()` per [observability-guidelines › logging](../../observability-guidelines/references/logging.md).
...
Log Hygiene review focuses on critical-severity cases where a log line interpolates a secret (token, password, session ID, full request body). Cross-reference with [application-security-requirements › secret-handling](../../application-security-requirements/references/secret-handling.md).
