...
Suspense Granularity review focuses on major-severity cases where a single `<Suspense>` boundary wraps two independently-slow async children. Split into one boundary per slow unit so each can stream to the client as it resolves.
...
Loading / Loaded Split review focuses on major-severity cases where a new component performs data fetching but does not split into `<name>/loaded.tsx` + `<name>/loading.tsx` + `<name>.tsx` orchestrator when the loading state is user-visible. The existing `app/(app)/_/components/webembed/` is the canonical layout.
...
Client Component Promotion review focuses on major-severity cases where the diff adds `"use client"` to a component whose only client-needing reason is a small interactive subtree (e.g., one button). Extract the interactive part into its own Client Component and keep the parent as a Server Component.
...
- MUST account for React Compiler behavior before flagging or approving manual memoization in new Client Components.
...
- MUST NOT recommend disabling `cacheComponents` or `reactCompiler` to "fix" a perf issue — escalate to the human owner per [code-review-guideline › escalation](../code-review-guideline/escalation.md).
