...
Placement of try-catch sets the required project default: place `try-catch` blocks at the **root call site** — the outermost function that initiates an operation (e.g., a route handler, a React Server Component, or an API route).
...
Next.js Error Boundaries sets the required project default: keep `app/(app)/global-error.tsx` as the last-resort error boundary for the entire application. This file MUST call `captureException(error)` inside a `useEffect` so that unexpected React render errors are also reported to Sentry.
...
Error Messages describes the preferred project default: write error messages that identify the exact function or condition that failed, so Sentry issues are immediately actionable without reading the stack trace.
