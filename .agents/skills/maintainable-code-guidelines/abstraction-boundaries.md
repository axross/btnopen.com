...
Server / Client Boundary sets the required project default: flag a `"use client"` component that performs data fetching (`fetch()`, `getPayload(…)`, `getBlogPost(…)`) — see [react-component-guidelines › client-vs-server-components](../react-component-guidelines/client-vs-server-components.md). Lift the fetch into the parent Server Component or its repository.
