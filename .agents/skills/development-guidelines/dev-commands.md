...
Apply this reference when choosing which npm script to run or when updating the command surface in `package.json`. The project expects Node.js `>=24.0.0` and npm `>=10.0.0` per `package.json`.
