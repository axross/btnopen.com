...
Payload `users` Collection review focuses on critical-severity cases where the diff weakens any of these in `payload/collections/user.ts`:
...
Session Cookies review focuses on critical-severity cases where a new component or route handler reads or writes session cookies directly via `cookies()` from `next/headers`. Payload owns cookie management — bypassing it desyncs the auth state.
...
Live Preview Auth Path review focuses on critical-severity cases where the diff makes `?preview=true` or `?draft=true` reachable without a valid Payload session. The existing `posts/[slug]/page.tsx` only renders `PayloadLivePreview` when `preview === "true"` — the draft data fetch in `getBlogPost({ draft: true })` still relies on Payload's draft-API authentication.
...
Localhost / Production Divergence review focuses on major-severity cases where the diff causes a code path to execute only when `isLocalhost === true` (per `app/(app)/_/runtime.ts`) but no equivalent exists for production — a localhost-only auth bypass that ships to production via deployed branch is a recurring class of bug.
