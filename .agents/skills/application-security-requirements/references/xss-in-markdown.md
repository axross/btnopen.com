...
- MUST review markdown-pipeline changes against the current dangerous-protocol, unknown-handler, and `rehypeReact` encoding guarantees.

## What the Reviewer MUST Flag

What the Reviewer MUST Flag review focuses on critical-severity cases where a new component added to `defaultComponents` in `app/(app)/_/components/markdown.tsx` uses `dangerouslySetInnerHTML` for any prop derived from the markdown content.
...
`webembed` and Future Directives review focuses on critical-severity cases where a new directive's React component renders unescaped HTML from any `node.attributes` or `node.children` value.
...
Image Sources review focuses on critical-severity cases where a new component renders `<Image src={userControlled} unoptimized />` with `userControlled` originating from CMS markdown without an allowlist check. `unoptimized` bypasses Next's `images.remotePatterns` validation, which is the only host gate.
...
React-Specific Pitfalls review focuses on critical-severity cases where jSX uses a key derived from CMS content and that key is the only barrier between two list items (e.g., `<li key={post.slug}>`). React itself encodes the key, but if the same value is also rendered as `<a href={post.slug}>` without URL-canonicalization, both flow to the DOM.
...
Bypass Paths review focuses on critical-severity cases where any new code calls `unified().use(…).process(rawHtml)` with HTML — the project uses markdown only. A divergence to HTML widens the attack surface to anything `rehype` accepts.
...
- MUST flag a Critical when a new module loads markdown from the filesystem at runtime — the project rule per [markdown-processing-guidelines › content-source](../../markdown-processing-guidelines/references/content-source.md) is that all content comes from Payload.
