...
Draft / Published Gating review focuses on critical-severity cases where a repository function accepts a `draft: boolean` parameter and passes `draft: true` to `payload.find(…)` without first verifying the request is authenticated to view drafts. Current callers gate on `searchParams.draft === "true"` AND rely on Payload's draft-API auth — confirm both are present.
...
Auth Settings on `users` review focuses on critical-severity cases where the diff weakens `payload/collections/user.ts`'s auth settings:

**Guidelines:**

- MUST flag a Critical when the diff weakens `payload/collections/user.ts`'s auth settings:
...
Live Preview / Preview Mode review focuses on critical-severity cases where a new route uses `?preview=true` to bypass production gating without also requiring `?draft=true` and Payload auth. The existing `posts/[slug]/page.tsx` checks `preview === "true"` only to render the `PayloadLivePreview` component — preview mode does not itself unlock draft content.
...
Admin UI Surface is a project prohibition: do not review files under `app/(payload)/` per [code-review-guideline › scoping](../../code-review-guideline/references/scoping.md). Payload owns that route segment.
