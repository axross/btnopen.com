...
Flagging Assumptions sets the required project default: state assumptions explicitly when the reviewer cannot verify a fact from the diff alone (e.g., "Assuming `BLOB_PAYLOAD_READ_WRITE_TOKEN` is set in production; if not, this branch will never run.").
...
Japanese/English Content is a project prohibition: do not correct or comment on the Japanese / English wording of CMS-managed content. Content correctness is a separate responsibility, not code review. From the code review perspective, SHOULD only flag rendering, escaping, or i18n-handling bugs in the surrounding code.
